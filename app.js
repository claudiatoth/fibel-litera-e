/* ===========================================================
   FIBEL — „Litera E cu Herzchen" · Claudia Toth © 2026
   Abecedar german interactiv pentru copii 4–8 ani.
   Temă: Elefant 🐘 · „E wie Elefant" · „Herzchen reitet auf dem Elefanten".

   16 secțiuni: Start · Litera E · Suche E · Cuvinte · Citește (cuvinte+propoziții) ·
   Legare silabe (tap-tap) · Joc E · Wo hörst du E · der/die/das · Povestea lui Herzchen ·
...
const E = {
  elefant: {de:'der Elefant', icon:'elefant', audio:'e-elefant', audioArt:'e-art-elefant', gen:'m', art:'der', ro:'elefantul', m:true},
  ente:    {de:'die Ente',    icon:'ente',    audio:'e-ente', audioArt:'e-art-ente',    gen:'f', art:'die', ro:'rața',       m:true},
  esel:    {de:'der Esel',    icon:'esel',    audio:'e-esel', audioArt:'e-art-esel',    gen:'m', art:'der', ro:'măgarul',    m:true},
  engel:   {de:'der Engel',   icon:'engel',   audio:'e-engel', audioArt:'e-art-engel',   gen:'m', art:'der', ro:'îngerul',    m:true},
  erde:    {de:'die Erde',    icon:'erde',    audio:'e-erde', audioArt:'e-art-erde',    gen:'f', art:'die', ro:'pământul',   m:true},
  essen:   {de:'das Essen',   icon:'essen',   audio:'e-essen', audioArt:'e-art-essen',   gen:'n', art:'das', ro:'mâncarea',   m:true},
  maus:  {de:'die Maus',  icon:'m-maus',audio:'e-maus',  gen:'f', art:'die', ro:'șoricelul', m:false},
  mond:  {de:'der Mond',  icon:'m-mond',audio:'e-mond',  gen:'m', art:'der', ro:'luna',      m:false},
  ball:  {de:'der Ball',  icon:'ball',  audio:'e-ball',  gen:'m', art:'der', ro:'mingea',    m:false},
  katze: {de:'die Katze', icon:'katze', audio:'e-katze', gen:'f', art:'die', ro:'pisica',    m:false},
  apfel: {de:'der Apfel', icon:'apfel', audio:'e-apfel', gen:'m', art:'der', ro:'mărul',     m:false},
  obst:  {de:'das Obst',  icon:'obst',  audio:'e-obst',  gen:'n', art:'das', ro:'fructele',  m:false},
  tiger: {de:'der Tiger', icon:'tiger', audio:'e-tiger', gen:'m', art:'der', ro:'tigrul',    m:false},
};
const WORDS_E  = ['elefant','ente','esel','engel','erde','essen'];
const GAME_POOL= ['elefant','maus','ente','ball','esel','katze'];
const ART_QUIZ = ['ente','esel','erde','essen'];
const GRID = [
  ['E','m','e','N','b'],['l','e','M','E','n'],['E','o','e','d','E'],['t','E','a','e','B'],['d','e','M','E','w'],
];
const STORY = [
  {de:'Das ist <b class="m-bold">Herzchen</b>.',                                    ro:'Acesta e Herzchen (inimioara).',   icon:'herzchen', audio:'st-1'},
  {de:'<b class="m-bold">Herzchen</b> sieht einen <b class="m-bold">Elefanten</b>.', ro:'Herzchen vede un elefant.',        icon:'elefant',  audio:'st-2'},
  {de:'<b class="m-bold">Herzchen</b> reitet auf dem <b class="m-bold">Elefanten</b>.',ro:'Herzchen călărește pe elefant.', icon:'elefant',  audio:'st-3'},
  {de:'<b class="m-bold">Herzchen</b> ist glücklich.',                              ro:'Herzchen e fericit.',              icon:'herzchen', audio:'st-4'},
];
const QUIZ = [
  {type:'audio',   ans:'elefant', opts:['elefant','maus','ball']},
  {type:'findm',   ans:'ente',    opts:['ente','katze','mond']},
  {type:'article', word:'essen'},
  {type:'audio',   ans:'engel',   opts:['engel','apfel','tiger']},
  {type:'findm',   ans:'esel',    opts:['esel','ball','obst']},
];
const LETTER = 'E';
const POS = [
  {icon:'elefant', audio:'e-elefant', pos:'a'},
  {icon:'ente',    audio:'e-ente',    pos:'a'},
  {icon:'esel',    audio:'e-esel',    pos:'a'},
  {icon:'engel',   audio:'e-engel',   pos:'a'},
];
...
SCREENS[0] = function(){
  ...
  show(
    ...
    '<h1 class="title">Litera E cu Herzchen</h1>' +
  ...
  document.getElementById('startBtn').onclick = ()=>{ play('e-wie-elefant'); go(1); };
...
SCREENS[1] = function(){
  show(
    ...
    '<div class="letter-hero">E<span class="lo">e</span></div>' +
    '<div class="btn-row">' +
      '<button class="big-btn" id="bName">🔊 E</button>' +
      '<button class="big-btn amber" id="bMaus">🔊 E wie Elefant</button>' +
    '</div>' +
    '<p class="parent-note">Atinge: „E" — se citește „e" · „E wie Elefant" = E ca în Elefant.</p>' +
    nextBtn()
  );
  document.getElementById('bName').onclick = ()=>play('e-name');
  document.getElementById('bMaus').onclick = ()=>play('e-wie-elefant');
  setTimeout(()=>play('e-wie-elefant'), 350);
  wireNext(2);
};
...
SCREENS[2] = function(){
  ...
  GRID.forEach(row=> row.forEach(ch=>{
    const isM = (ch==='E'||ch==='e'); if(isM) total++;
    cells += '<div class="lcell" data-m="'+(isM?1:0)+'">'+ch+'</div>';
  }));
  show(
    '<p class="kicker">Suche E und e 🔍</p>' +
    '<h1 class="title" style="font-size:1.4rem">Atinge toți E și e</h1>' +
...
SCREENS[3] = function(){
  ...
  show(
    ...
    '<h1 class="title" style="font-size:1.5rem">Cuvinte cu E</h1>' +
    '<div class="cards">'+cards+'</div>' + nextBtn()
  );
  stage.querySelectorAll('.card').forEach(el=>{ el.onclick = ()=>{ el.classList.add('tapped'); play(E[el.dataset.k].audioArt||E[el.dataset.k].audio); }; });
  wireNext(4);
};
...
SCREENS[4] = function(){
  const READ = [
    {t:'Ente', a:'e-art-ente'}, {t:'Esel', a:'e-art-esel'}, {t:'Emil', a:'e-emil'},
    {t:'Nina', a:'e-nina'}, {t:'Oma', a:'e-oma'}, {t:'Tom', a:'e-tom'}
  ];
  const SENT = [
    {t:'Emil reitet.', a:'st-emilreitet'}, {t:'Nele reitet.', a:'st-nelereitet'}
  ];
  ...
  show(
    ...
    '<h1 class="title" style="font-size:1.1rem">Cu literele învățate + E</h1>' +
...
/* 5 — LEGARE SILABE */
SCREENS[5] = function(){
  const LEFT  = ['E','No','Ni','O'];
  const RIGHT = ['sel','ra','na','ma'];
  const VALID = {
    'E|sel':  {w:'Esel', a:'e-esel'},
    'No|ra':  {w:'Nora', a:'e-nora'},
    'Ni|na':  {w:'Nina', a:'e-nina'},
    'O|ma':   {w:'Oma',  a:'e-oma'}
  };
...
/* 6 — JOC: beginnt mit E? */
SCREENS[6] = function(){
  let tiles = spreadShuffle(GAME_POOL, k=>E[k].m, 2).map(k=>
    '<div class="tile" data-k="'+k+'" style="position:relative"><img src="'+ICON+E[k].icon+'.svg" alt="'+E[k].de+'"></div>'
  ).join('');
  show(
    hero('small') +
    '<p class="game-q">Atinge ce începe cu <span class="m-hi">E</span>!</p>' +
    '<div class="game-grid">'+tiles+'</div>' +
    '<p class="parent-note">Welches Bild beginnt mit E?</p>'
  );
  let found = 0; const need = GAME_POOL.filter(k=>E[k].m).length;
  stage.querySelectorAll('.tile').forEach(el=>{
    el.onclick = function(){
      const w = E[el.dataset.k];
  ...
  function card(){
    ...
    show(
      ...
      '<div class="flashnav">' +
        ...
        '<span class="counter">'+(i+1)+' / '+WORDS_E.length+'</span>' +
    ...
    pv.disabled = (i<=0); nx.disabled = (i>=WORDS_E.length-1);
    pv.onclick = ()=>{ if(i>0){ i--; card(); } };
    nx.onclick = ()=>{ if(i<WORDS_E.length-1){ i++; card(); } };
    wireNext(11);
  }
  card();
};

/* 11 — TRASEAZA E / e — cu SĂGEȚI */
SCREENS[11] = function(){
  let glyph = 'E';
  const STROKE = {'E':'stroke-big','e':'stroke-small'};
  function render(){
    show(
      '<p class="kicker">Schreiben · ① urmează săgețile verzi ✏️</p>' +
      '<div class="btn-row" style="margin-bottom:2px">' +
        '<button class="big-btn'+(glyph==='E'?'':' amber')+'" id="gM">E mare</button>' +
        '<button class="big-btn'+(glyph==='e'?'':' amber')+'" id="gm">e mic</button></div>' +
    ...
    document.getElementById('gM').onclick=()=>{glyph='E';render();};
    document.getElementById('gm').onclick=()=>{glyph='e';render();};
...
SCREENS[13] = function(){
  ...
  const SENT = [
    {t:'Emil reitet.', a:'st-emilreitet'},
    {t:'Nele reitet.', a:'st-nelereitet'}
  ];
  ...
  function q(){
    ...
    if (it.type==='audio'){
      body = '<p class="game-q">Ascultă 🔊 și atinge imaginea</p>' +
        ...
        '<div class="opt-grid">'+ shuffle(it.opts).map(k=>'<div class="opt" data-k="'+k+'"><img src="'+ICON+E[k].icon+'.svg" alt=""></div>').join('') +'</div>';
    } else if (it.type==='findm'){
      body = '<p class="game-q">Care începe cu <span class="m-hi">E</span>?</p>' +
        '<div class="opt-grid">'+ shuffle(it.opts).map(k=>'<div class="opt" data-k="'+k+'"><img src="'+ICON+E[k].icon+'.svg" alt=""></div>').join('') +'</div>';
    } else {
      const w = E[it.word];
    ...
    if (it.type==='audio'){
      ...
      stage.querySelectorAll('.opt').forEach(el=> el.onclick = function(){
        if (el.dataset.k === it.ans){ el.classList.add('good'); play(E[el.dataset.k].audio); win(); } else wrong(el);
      });
    } else if (it.type==='findm'){
      stage.querySelectorAll('.opt').forEach(el=> el.onclick = function(){
        if (E[el.dataset.k].m){ el.classList.add('good'); play(E[el.dataset.k].audio); win(); } else wrong(el);
      });
    } else {
      const w = E[it.word]; play(w.audio);
...
SCREENS[15] = function(){
  show(
    ...
    '<div class="story" style="max-width:380px"><div class="story-row read" id="rima">' +
      '<img src="'+ICON+'elefant.svg" alt="">' +
      '<div><div class="s-de"><b class="m-bold">E</b> wie <b class="m-bold">Elefant</b>.<br><b class="m-bold">Herzchen</b> reitet auf dem <b class="m-bold">Elefanten</b>.</div>' +
      '<div class="s-ro">E ca în Elefant. Herzchen călărește pe elefant.</div></div>' +
...
function printDiploma(){
  ...
  document.getElementById('diploma-print').innerHTML =
    '<div class="dip">' +
      ...
      '<div class="dip-letter">E e</div>' +
      '<div class="dip-text">E wie Elefant — mit Herzchen! 🧡</div>' +
...
function printColoring(){
  ...
  document.getElementById('coloring-print').innerHTML =
    '<div class="color-page">' +
      ...
      '<h2 class="cp-title">E — Herzchen reitet auf dem Elefanten</h2>' +
      '<img src="'+ICON+'herzchen-color.svg" alt="Herzchen reitet auf dem Elefanten">' +
      '<div class="cp-sentence">Herzchen reitet auf dem Elefanten.</div>' +
...
(function initParent(){
  ...
  if (wl) wl.innerHTML = WORDS_E.map(k=>'<span>'+E[k].de+'</span>').join('');
